...
const generateBoardId = (email) => {
  const hash = Array.from(email).reduce((acc, c) => acc + c.charCodeAt(0), 0)
  return `board-${hash}-${email.replace(/[^a-z0-9]/gi, '').slice(0, 8)}`
}
...
export default function AuthGate({ children }) {
  ...
  const handleLogin = () => {
    ...
    setLoading(true)
    setTimeout(() => {
      const session = { name: u.name, email: u.email, boardId: u.boardId }
      saveSession(session)
      setUser(session)
      setLoading(false)
    }, 600)
  ...
  const handleSignup = () => {
    ...
    const users = getUsers()
    if (users[form.email.toLowerCase()]) { setError('An account with this email already exists.'); return }
    setLoading(true)
    setTimeout(() => {
      const boardId = generateBoardId(form.email)
      const newUser = { name: form.name.trim(), email: form.email.toLowerCase(), password: form.password, boardId }
      users[form.email.toLowerCase()] = newUser
      saveUsers(users)
      const session = { name: newUser.name, email: newUser.email, boardId }
      saveSession(session)
      setUser(session)
      setLoading(false)
    }, 700)
