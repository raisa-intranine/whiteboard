...
const PresenceIndicators = ({ containerRef }) => {
  ...
  useEffect(() => {
    ...
    const applyMember = (event, member) => {
      const myClientId = getClientId()
      console.log('[PresenceIndicators] Presence event:', event, 'member:', member.clientId, 'name:', member.data?.name, 'isEditing:', member.data?.isEditing)
      if (event === 'enter' || event === 'update') {
        setPresenceMembers(prev => {
          ...
          console.log('[PresenceIndicators] Adding/updating member:', member.data?.name, 'isEditing:', member.data?.isEditing)
  ...
  // Debug: log editing members whenever they change
  useEffect(() => {
    console.log('[PresenceIndicators] editingMembers calculation:')
    console.log('[PresenceIndicators]   - Total members:', presenceMembers.length)
    console.log('[PresenceIndicators]   - Editing members:', editingMembers.length)
    presenceMembers.forEach(m => {
      console.log('[PresenceIndicators]   - Member:', m.data?.name, 'isEditing:', m.data?.isEditing, 'full data:', m.data)
    })
  }, [editingMembers.length, presenceMembers.length])
