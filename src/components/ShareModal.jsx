...
const ShareModal = ({ visible, onClose, boardId, theme }) => {
  ...
  const handleAddByEmail = async (email) => {
    ...
    try {
      ...
      if (result.invited) {
        ...
        try {
          await sendBoardInvite(email, boardId, user.displayName || user.email, user.email)
          setNotification({
            type: 'success',
          ...
          setNotification({
            type: 'success',
            message: `${email} added to board (email notification failed)`
          })
        }
      } else {
        setNotification({
          type: 'success',
          message: `${email} has been added to the board!`
        })
      }
  ...
  const confirmRemoveInvite = async () => {
    ...
    setLoading(true)
    try {
      await removePendingInvite(boardId, removingInviteId)
      await loadCollaborators()
      setNotification({
        type: 'success',
  ...
  return (
    <>
        ...
        <div className="share-modal-body">
          ...
          <div className="share-section">
            ...
            {searchQuery.length >= 2 && !searching && searchResults.length === 0 && (
              <div className="share-no-results-with-action">
                <div className="share-no-results-text">No users found</div>
                {searchQuery.includes('@') && (
                  <button
                    className="share-add-by-email-btn"
                    onClick={() => handleAddByEmail(searchQuery)}
                    disabled={loading}
                  >
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
          ...
          <div className="share-section">
            ...
            <div className="share-collaborators-list">
              ...
              {pendingInvites.map(invite => {
                const [grad1, grad2] = getAvatarColor(invite.email)
                return (
                  <div key={invite.id} className="share-collaborator pending">
                    <div className="share-user-avatar" style={{ background: `linear-gradient(135deg, ${grad1}, ${grad2})`, opacity: 0.6 }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                        <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
                        <polyline points="22,6 12,13 2,6" />
                      </svg>
      ...
      {(removingUserId || removingInviteId) && (
        <div className="share-confirm-backdrop" onClick={() => { setRemovingUserId(null); setRemovingInviteId(null) }}>
          <div className={`share-confirm-dialog ${theme}`} onClick={e => e.stopPropagation()}>
            <div className="share-confirm-header">
              ...
              <h3>{removingUserId ? 'Remove this collaborator?' : 'Cancel this invite?'}</h3>
            </div>
            <p className="share-confirm-message">
              {removingUserId 
                ? 'They will no longer be able to access this board.'
                : 'The pending invitation will be cancelled.'}
            </p>
            <div className="share-confirm-actions">
              <button 
                className="share-confirm-btn cancel"
                onClick={() => { setRemovingUserId(null); setRemovingInviteId(null) }}
                disabled={loading}
              >
                Cancel
              </button>
              <button 
                ...
                onClick={removingUserId ? confirmRemove : confirmRemoveInvite}
                disabled={loading}
              >
